...
public:
   ...
   * splitting lines. It will also call helper function buildTree.
   ...
   void render_recur(PNG & imIn, Node*  root);
   ...
   * You may want a recursive helper function for this one.
   ...
   void prune_recur(double pct, int tol, Node* & root);

   long prune_count(double pct, int tol, Node* & root, Node* & child);
...
private:
   ...
   */
   void copy(const twoDtree & orig);

   twoDtree::Node * copytree(const Node* root);
  ...
  void build_rec(stats & s, pair<int,int> ul, pair<int,int> lr, Node*& n);
